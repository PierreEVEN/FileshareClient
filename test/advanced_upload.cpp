...
TEST(AdvancedTransfer, WeirdNames)
{
    ...
    while (start < max) {
        ...
        for (size_t i = start; i < end; ++i) {
            if (forbidden_chars.find(i) != std::wstring::npos)
                continue;
        ...
        for (const auto& diffs : diff.get_changes())
            std::cout << diffs.operation_str() << " " << fileshare::Url::encode_string(diffs.get_file().get_path().generic_wstring()) << std::endl;
...
TEST(AdvancedTransfer, UploadBigFile)
{
    auto& test_env = FileshareTestEnvironment::get_connected();
    ...
    const auto data1 = test_env.edit_file(test_env.get_test_data_dir() / "test_dir" / "test_file.bin", 100 * 1024 * 1024); // 100Mo
